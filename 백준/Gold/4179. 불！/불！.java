// 1. 불을 먼저 bfs로 측정하고, 지훈이를 움직인다.
// 2. 시간을 기준으로 지훈이와 불을 동시에 움직인다.(그래도 불 먼저 확산하고 지훈이가 움직인다.) -> 지훈이가 먼저 움직이면
//    불이 그 후에 붙기 때문

import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStreamReader;
import java.util.ArrayDeque;
import java.util.ArrayList;
import java.util.Deque;
import java.util.List;
import java.util.StringTokenizer;

public class Main {

	public static void main(String[] args) throws IOException {
		BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
		StringTokenizer st = new StringTokenizer(br.readLine());
		int R = Integer.parseInt(st.nextToken());
		int C = Integer.parseInt(st.nextToken());
		int[][] map = new int[R][C];
		Deque<int[]> fire = new ArrayDeque<>(); // 불의 위치를 담을 덱
		Deque<int[]> dq = new ArrayDeque<>(); // 현재 위치를 담을 덱
		for (int r = 0; r < R; r++) {
			String str = br.readLine();
			for (int c = 0; c < C; c++) {
				switch (str.charAt(c)) {
				case '#':
					map[r][c] = -1; // 벽은 -1로
					break;
				case '.':
					map[r][c] = 0; // 지나갈 수 있는 곳은 0으로
					break;
				case 'J':
					map[r][c] = 0;
					dq.offerLast(new int[] {r, c});
					if (r == 0 || r == R-1 || c == 0 || c == C-1) {
						System.out.println(1); // 밖으로 바로 탈출 가능
						return;
					}
					break;
				case 'F':
					map[r][c] = -1; // 불난 곳은 1로
					fire.offerLast(new int[] {r, c});
					break;
				}
			}
		}
		int time = 0;
		int[] dr = {-1, 1, 0, 0};
		int[] dc = {0, 0, -1, 1};
		int size = 0;
		while (!dq.isEmpty()) {
			time++;
			size = fire.size(); // 불 먼저 확산
			for (int i = 0; i < size; i++) {
				int[] tmp = fire.pollFirst();
				for (int idx = 0; idx < 4; idx++) {
					int nr = tmp[0] + dr[idx];
					int nc = tmp[1] + dc[idx];
					if (nr >= 0 && nr < R && nc >= 0 && nc < C && map[nr][nc] == 0) {
						map[nr][nc] = -1;
						fire.offerLast(new int[] {nr, nc});
					}
				}
			}
			size = dq.size(); // 지훈이 이동
			for (int i = 0; i < size; i++) {
				int[] tmp = dq.pollFirst();
				for (int idx = 0; idx < 4; idx++) {
					int nr = tmp[0] + dr[idx];
					int nc = tmp[1] + dc[idx];
					if (nr >= 0 && nr < R && nc >= 0 && nc < C && map[nr][nc] == 0) {
						map[nr][nc] = time;
						if (nr == 0 || nr == R-1 || nc == 0 || nc == C-1) {
							System.out.println(time+1); // 밖으로 나가야하므로 한 번 더 움직여야 한다.
							return;
						}
						dq.offerLast(new int[] {nr, nc});
					}
				}
			}
		} // bfs
		System.out.println("IMPOSSIBLE");
		
	} // main
}